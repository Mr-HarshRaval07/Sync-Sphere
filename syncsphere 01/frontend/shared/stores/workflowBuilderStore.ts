...
interface WorkflowBuilderState {
  ...
  // Actions - Canvas
  initBuilder: (workflowId: string, name: string, description: string, nodes: Node<NodeConfig>[], edges: Edge[], state: WorkflowState) => void;
  resetBuilder: () => void;
...
export const useWorkflowBuilderStore = create<WorkflowBuilderState>((set, get) => ({
  ...
  // --- Canvas Actions ---
  initBuilder: (workflowId, name, description, nodes, edges, state) => {
    set({
      workflowId,
      workflowName: name,
  ...
  // --- Versioning Actions ---
  saveDraft: () => {
    const { nodes, edges, versions, activeVersion, workflowName } = get();
    const newVersion: WorkflowVersion = {
      version: activeVersion + 1,
      nodes: structuredClone(nodes),
      edges: structuredClone(edges),
      createdAt: new Date().toISOString(),
      label: `Draft v${activeVersion + 1}`,
      state: 'DRAFT',
    };
    set({
  ...
  publishWorkflow: () => {
    const { nodes, edges, versions, activeVersion } = get();
    const newVersion: WorkflowVersion = {
      version: activeVersion + 1,
      nodes: structuredClone(nodes),
      edges: structuredClone(edges),
      createdAt: new Date().toISOString(),
      label: `Published v${activeVersion + 1}`,
      state: 'PUBLISHED',
    };
    set({
      versions: [...versions, newVersion],
