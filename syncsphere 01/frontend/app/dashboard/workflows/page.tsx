...
import { workflowApi, runtimeApi } from '../../../shared/services/api';
...
const BuilderView: React.FC<BuilderViewProps> = ({ workflowId, workflowName, onBack }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: () => workflowApi.updateWorkflow(workflowId, {
      name: useWorkflowBuilderStore.getState().workflowName,
      description: useWorkflowBuilderStore.getState().workflowDescription,
      nodes: nodes.map((n) => ({
        id: n.id,
    ...
    onSuccess: () => {
      saveDraft();
      toast.success('Saved', { description: 'Workflow draft saved successfully.' });
    },
    onError: (err: any) => {
      // Fallback to local save
      saveDraft();
      toast.info('Saved Locally', { description: 'Saved to local versioning. API sync unavailable.' });
    },
  });

  const handlePublish = useCallback(() => {
    publishWorkflow();
    toast.success('Published', { description: 'Workflow published and ready for execution.' });
  }, [publishWorkflow]);
      ...
      <div className="flex h-12 items-center justify-between border-b border-border bg-muted/30 px-3 shrink-0 gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-7 px-3 rounded-md text-xs font-bold bg-primary text-primary-foreground hover:bg-primary/90 transition-colors"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" role="dialog" aria-label="Select a template">
      ...
      <div className="relative bg-card border border-border rounded-xl shadow-2xl w-full max-w-2xl max-h-[70vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 gap-3 scrollbar-thin">
          {/* Blank workflow */}
          <button
            onClick={() => onSelect({ id: 'blank', name: 'Blank Workflow', description: 'Start from scratch with an empty canvas.', category: 'General', nodes: [], edges: [] })}
...
export default function WorkflowsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: any) => workflowApi.createWorkflow(payload),
    onSuccess: (data: any) => {
      queryClient.invalidateQueries({ queryKey: ['workflows-list'] });
      toast.success('Workflow Created', { description: 'Opening workflow builder...' });
      handleOpenBuilder(data.id || `wf_${Date.now()}`, data.name || 'New Workflow', '', [], []);
    },
    onError: () => {
      // Fallback: open builder locally
      const tempId = `local_wf_${Date.now()}`;
      handleOpenBuilder(tempId, 'New Workflow', '', [], []);
      toast.info('Created Locally', { description: 'Opened builder with local state.' });
    },
  });
  ...
  const runMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success('Execution Started', {
        description: `Run session #${data.id.slice(-6)} dispatched.`,
      });
    },
    onError: (err: any) => {
  ...
  const handleOpenBuilder = useCallback((id: string, name: string, description: string, nodes: Node<NodeConfig>[], edges: Edge[]) => {
    // Check for autosave first
    const autosaved = loadAutosave(id);
    if (autosaved && autosaved.nodes?.length > 0) {
      initBuilder(id, autosaved.workflowName || name, autosaved.workflowDescription || description, autosaved.nodes, autosaved.edges, autosaved.workflowState || 'DRAFT');
      toast.info('Autosave Restored', { description: 'Loaded from your last session.' });
    } else {
      initBuilder(id, name, description, nodes, edges, 'DRAFT');
    }
  ...
  const handleSelectWorkflow = useCallback((workflowId: string) => {
    ...
    const flowNodes: Node<NodeConfig>[] = (wf.nodes || []).map((n: any, idx: number) => ({
      id: n.id,
      type: n.type || 'start',
      position: { x: 80 + idx * 280, y: 150 },
      data: {
        label: n.label || n.id,
    ...
    const flowEdges: Edge[] = (wf.edges || []).map((e: any, idx: number) => ({
      id: `e-${idx}`,
      source: e.source,
      target: e.target,
      animated: true,
    ...
    handleOpenBuilder(wf.id, wf.name, wf.description || '', flowNodes, flowEdges);
  ...
  const handleTemplateSelect = useCallback((template: WorkflowTemplate) => {
    ...
    // For template, use original IDs (no timestamp suffix needed since they're unique per template)
    const nodes = template.nodes as Node<NodeConfig>[];
    ...
    // Also try to create on API
    createMutation.mutate({ name: template.name, description: template.description, nodes: [], edges: [] });
    handleOpenBuilder(tempId, template.name, template.description, nodes, edges);
  }, [createMutation, handleOpenBuilder]);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              const tempId = `wf_${Date.now()}`;
              handleOpenBuilder(tempId, 'Untitled Workflow', '', [], []);
            }}
