...
export const WORKFLOW_TEMPLATES: WorkflowTemplate[] = [
  {
    ...
    nodes: [
      ...
      makeNode('connector_1', 'connector', 'Create Jira P0', 980, 60, { connector_id: 'jira', tool_name: 'create_ticket' }),
      makeNode('connector_2', 'connector', 'Create Jira P2', 980, 240, { connector_id: 'jira', tool_name: 'create_ticket' }),
      makeNode('connector_3', 'connector', 'Slack Notify', 1280, 150, { connector_id: 'slack', tool_name: 'post_message' }),
  ...
  },
  {
    ...
    description: 'AI-assisted pull request review, security scanning, and automated feedback posting.',
    category: 'Engineering',
    nodes: [
      makeNode('start_1', 'start', 'PR Webhook', 80, 150),
      makeNode('connector_1', 'connector', 'Fetch PR Diff', 380, 150, { connector_id: 'github', tool_name: 'get_pull_request' }),
      makeNode('ai_1', 'ai', 'Review Code', 680, 80, { model_id: 'claude-3-5-sonnet', prompt_template_id: 'code_review' }),
      makeNode('ai_2', 'ai', 'Security Scan', 680, 240, { model_id: 'gpt-4o', prompt_template_id: 'security_scan' }),
      makeNode('connector_2', 'connector', 'Post Comments', 980, 150, { connector_id: 'github', tool_name: 'create_review' }),
      makeNode('end_1', 'end', 'Review Done', 1280, 150),
    ],
    edges: [
      { source: 'start_1', target: 'connector_1' },
      { source: 'connector_1', target: 'ai_1' },
      { source: 'connector_1', target: 'ai_2' },
      { source: 'ai_1', target: 'connector_2' },
      { source: 'ai_2', target: 'connector_2' },
      { source: 'connector_2', target: 'end_1' },
    ],
  },
  {
    ...
    nodes: [
      ...
      makeNode('connector_1', 'connector', 'Fetch Jira Stats', 380, 80, { connector_id: 'jira', tool_name: 'get_sprint_summary' }),
      makeNode('connector_2', 'connector', 'Fetch GitHub Stats', 380, 240, { connector_id: 'github', tool_name: 'get_commit_stats' }),
      makeNode('ai_1', 'ai', 'Generate Summary', 680, 150, { model_id: 'gpt-4o', prompt_template_id: 'daily_summary' }),
      makeNode('connector_3', 'connector', 'Post to Slack', 980, 80, { connector_id: 'slack', tool_name: 'post_message' }),
      makeNode('http_1', 'http', 'Send Email', 980, 240, { url: '/api/send-email', method: 'POST' }),
