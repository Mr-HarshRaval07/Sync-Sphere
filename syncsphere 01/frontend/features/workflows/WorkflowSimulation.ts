...
export function simulateWorkflow(
  ...
  for (const nodeId of executionOrder) {
    ...
    // Track connectors
    if (nodeType === 'connector' && node.data.config?.connector_id) {
  ...
  switch (nodeType) {
    ...
    case 'connector':
      return {
        tool_result: {
          is_error: false,
          content: `Simulated response from connector "${data.config?.connector_id || 'unknown'}" tool "${data.config?.tool_name || 'unknown'}"`,
        },
      };
    case 'approval':
      return {
