...
export const useAdminStore = create<AdminState>((set, get) => ({
  ...
  complianceItems: [
    ...
    { id: 'soc-2', framework: 'SOC2', controlName: 'Audit Logging & Monitoring', status: 'compliant', description: 'Trace logs retained for 90+ days.' },
    { id: 'iso-1', framework: 'ISO27001', controlName: 'Cryptography Secrets Rotation', status: 'warning', description: '3 connector API keys exceed 90-day cycle.' },
    { id: 'hipaa-1', framework: 'HIPAA', controlName: 'Data Encryption at Rest', status: 'compliant', description: 'Secrets encrypted using AES-256 GCM.' },
  ],
  orgFeatureFlags: {
  ...
  inspectPermission: (userId, resource, action) => {
    // Mock authorization evaluations logic
    const isAllowed = action === 'read' || userId.includes('admin') || resource.includes('public');
    set({
      permissionInspector: { userId, resource, action, allowed: isAllowed },
    });
  },
