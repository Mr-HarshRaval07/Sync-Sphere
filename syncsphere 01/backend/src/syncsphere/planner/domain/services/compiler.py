...
class ApprovalGateInserter:
    ...
    def inject_approval(graph: WorkflowGraph, risk_level: str) -> None:
        ...
        # Find any node requiring approval (e.g. destructive action, tools starting with delete/remove)
        target_node_ids = []
        for nid, node in list(graph.nodes.items()):
            if node.type == WorkflowStepType.TOOL_CALL:
                tool_name = node.tool_invocation.tool_name.lower()
                if any(term in tool_name for term in ["delete", "remove", "destroy", "prune"]):
        ...
        for target_id in target_node_ids:
            ...
            app_node = WorkflowNode(
                id=app_node_id,
                name=f"Approve {graph.nodes[target_id].name}",
                type=WorkflowStepType.APPROVAL,
                approval_gate=ApprovalGate(
                    approver_role_id="admin_role",
                    timeout_seconds=86400,
                    message=f"Please review and approve the destructive operation: {graph.nodes[target_id].name}"
                )
            )
            graph.nodes[app_node_id] = app_node
            
            # Redirect edges: any edge going into target_id now goes into app_node_id.
            # And add an edge from app_node_id to target_id.
