...
class AIGatewayImpl(AIGateway):
    ...
    async def generate_chat(
        ...
        try:
            ...
            logger.warning("Primary model %s failed: %s. Attempting fallback...", model.name, str(primary_error))
            ...
            if not fallback_model:
                raise ProviderOfflineException(provider.name, f"Primary failed: {str(primary_error)}. No fallback model configured.")
                
            fallback_provider = providers_dict[fallback_model.provider_id]
            
            # Check fallback circuit breaker
            if not self.circuit_breaker.can_execute(fallback_provider.name):
                raise ProviderOfflineException(fallback_provider.name, "Fallback provider circuit breaker is OPEN.")
                
            fallback_adapter = self.provider_registry.get(fallback_provider.name)
            if not fallback_adapter:
                raise ProviderOfflineException(fallback_provider.name, "Fallback adapter not registered.")
    ...
    async def generate_completion(
        ...
        try:
            ...
            logger.warning("Primary completion failed: %s. Re-routing fallback...", str(primary_error))
            self.circuit_breaker.record_failure(provider.name)
            ...
            fallback_model = fallback_handler.select(other_models, providers_dict)
            if not fallback_model:
                raise ProviderOfflineException(provider.name, f"Primary completion failed: {str(primary_error)}")
    ...
    async def stream_completion(
        ...
        from syncsphere.ai.domain.events import StreamingStarted, StreamingCompleted
