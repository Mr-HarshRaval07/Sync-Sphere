...
async def get_dashboard(org_id: str = Depends(get_org_id)):
    data = await container.observability_service.dashboard_pipeline.compile_dashboard(org_id)
    return ResponseEnvelope(
        data=data,
        meta=ResponseMeta(request_id="N/A", status="success")
    )
...
async def get_health(org_id: str = Depends(get_org_id)):
    query = HealthDashboardQuery(org_id=org_id)
