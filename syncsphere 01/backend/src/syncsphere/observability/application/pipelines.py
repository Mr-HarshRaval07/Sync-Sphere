...
class DashboardPipeline:
    ...
    async def compile_dashboard(self, org_id: str) -> Dict[str, Any]:
        ai_data = await self.ai.get_ai_analytics(org_id)
        conn_data = await self.conn.get_connector_analytics(org_id)
        plan_data = await self.plan.get_planner_stats(org_id)
        run_data = await self.run.get_runtime_stats(org_id)
        know_data = await self.know.get_knowledge_stats(org_id)
        appr_data = await self.appr.get_approval_stats(org_id)
        org_data = await self.org.get_org_stats(org_id)
        use_data = await self.use.get_usage_stats(org_id)
        cost_data = await self.cost.get_cost_stats(org_id)
