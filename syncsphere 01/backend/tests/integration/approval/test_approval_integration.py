...
def test_approval_api_lifecycle_and_integration():
    ...
    user_id, org_id = loop.run_until_complete(get_ids())
    ...
    import asyncio
    loop = asyncio.get_event_loop()
    loop.run_until_complete(update_assignee_in_db())
    ...
    loop.run_until_complete(save_entities_in_db())
    ...
    loop.run_until_complete(trigger_completed_event())
    ...
    loop.run_until_complete(check_resumed_session())
