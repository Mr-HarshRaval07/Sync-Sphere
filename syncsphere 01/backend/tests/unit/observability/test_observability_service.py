...
async def test_analytics_engines():
    ...
    await event_store.record_event("e1", "ai.completion_generated", org_id, "corr1", datetime.utcnow(), {"tokens": {"total_tokens": 120}, "cost": 0.005})
    # Connector event
    await event_store.record_event("e2", "connector.tool_called", org_id, "corr2", datetime.utcnow(), {"connector_id": "slack"})

    ai_stats = await ai_engine.get_ai_analytics(org_id)
    assert ai_stats["token_usage"] == 120
    assert ai_stats["total_cost"] == 0.005

    conn_stats = await conn_engine.get_connector_analytics(org_id)
    assert conn_stats["connector_usage"]["slack"] == 1
