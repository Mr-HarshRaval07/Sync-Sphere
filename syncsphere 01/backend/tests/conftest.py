...
class MockAIProvider:
    """
    Deterministic AI provider used only during tests.

    Prevents integration tests from calling real external AI
    providers and ensures tests remain fast and reproducible.
    """
...
# Mock Redis Pub/Sub completely so background Redis listeners
# never connect to a real Redis server during tests.
...
def clean_repositories():
    ...
    # IMPORTANT:
    # The integration tests register a provider named "mock".
    # Route that provider to the deterministic test implementation
    # instead of any real external AI provider.
    ...
    container.observability_event_store_service = EventStoreService(
    ...
    container.observability_conn_analytics = ConnectorAnalyticsEngine(
        container.observability_metric_repo,
        container.observability_event_store_repo,
    )
    ...
    container.observability_exe_replay = ExecutionReplayEngine(
        container.observability_replay_repo,
        container.observability_event_store_repo,
    )

    container.observability_wf_replay = WorkflowReplayEngine(
        container.observability_replay_repo,
        container.observability_event_store_repo,
    )

    container.observability_pl_replay = PlannerReplayEngine(
        container.observability_replay_repo,
        container.observability_event_store_repo,
    )
